...
public class AvoidAlgorithm implements IMoveAlgorithm {
    ...
    public AvoidAlgorithm(IMouse mouse, List<ISubway> subways, List<IPlayer> cats){
        this.mouse = mouse;
    ...
    private IPosition getRandomExitHolePosition(ISubway subway){
        ...
        for(IExit exit : subway.getExits()){
            IPosition closesCat = getClosestCatLastKnown(exit.getPosition());
            
            float distance = closesCat.subtract(exit.getPosition()).length();
        ...
        return bestExit.getPosition();
    }


    private IPosition getClosestCatLastKnown(IPosition position) {

        IPosition closestCat = new Position(1000000,1000000);

        for(IPosition catPosition : this.residingSubway.getCatPositions()){
            float lengthCurrentClosest = closestCat.subtract(position).length();
            float lengthNewClosest = catPosition.subtract(position).length();

            if(lengthNewClosest < lengthCurrentClosest){
                closestCat = catPosition;
            }
        }
    ...
    private IPosition getClosestCat() {
        List<IPosition> catPositions = this.cats.stream().map(c -> c.getPosition()).collect(Collectors.toList());
        IPosition closestCat = new Position(1000000,1000000);

        for(IPosition catPosition : catPositions){
            float lengthCurrentClosest = closestCat.subtract(this.mouse.getPosition()).length();
            float lengthNewClosest = catPosition.subtract(this.mouse.getPosition()).length();
    ...
    private IPosition calculateNextPosition(){
        ...
        IPosition closestCat = getClosestCat();

        if(closestCat.subtract(this.mouse.getPosition()).length() < 2f){
            IPosition direction = closestCat.subtract(this.mouse.getPosition()).multiply(-1);
            IPosition nextPositionForCat = direction.multiply(((speed * diff) / direction.length()));
            return nextPositionForCat.add(this.mouse.getPosition());
        }else {
            IPosition direction = this.goalHole.getPosition().subtract(this.mouse.getPosition());
            IPosition nextPositionForHole = direction.multiply(((speed * diff) / direction.length()));

            if (direction.length() < nextPositionForHole.length()) {
                return this.goalHole.getPosition();
            } else {
                return nextPositionForHole.add(this.mouse.getPosition());
            }
        }
    }
